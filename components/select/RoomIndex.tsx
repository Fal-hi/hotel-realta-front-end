...
const RoomIndex: React.FC<RoomIndexProps> = () => {
  ...
  const RoomIndexRef = useRef<HTMLDivElement | null>(null)
  ...
  const handleClickOutside = (event: any) => {
    if (RoomIndexRef.current && !RoomIndexRef.current.contains(event.target)) {
      setShowModal(false)
    }
  }
  ...
  return (
    <div
      ref={RoomIndexRef}
      className="px-2 py-1 w-40 text-md text-center text-[#323437] bg-white  rounded-md"
    >
      <button onClick={handleModalOpen} className="text-md text-[#323437]">
        {totalGuestCount}
      </button>
