...
import { doUpdatePode } from "@/redux/PURCHASING/action/actionPode"

export default function EditStockListOrder(props: any) {
  ...
  const handleEdit = async (data: FormValues) => {
    ...
    dispatch(doUpdatePode(props.isEdit.id,dataForm))
    props.closeModal()
  }
    ...
    const getData = async () => {
      setData(
        listOrder?.data?.data.filter(
          (listOrder: any) => listOrder.pode_id === props.isEdit.id
        )[0]
      )
    }
    getData()
